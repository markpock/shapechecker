...
def AddRel (a b : Py.Shape) : Prop := a = b
def tensorAddPrimOp (t : Tensor a) (t' : Tensor b) (r : AddRel a b) : Tensor a := .mk
...
macro_rules | `(tactic| resolve_prim_op_trivial) => `(tactic| simp (config := { arith := true }); done)
...
partial def expr (e : Py.Expr) : CommandElabM (TSyntax `term) := do
  ...
  | .call (.var n) args =>
    if isPrimOp n then `($(<- fncall (.var n) args) (by resolvePrimOp))
...
def tensorAddAlteration :
    Tensor (Py.Shape.var l) -> Tensor (Py.Shape.var l) -> Tensor (Py.Shape.var l) := λ a b =>
  ((tensorAddPrimOp b) a) (by resolvePrimOp)


set_option pp.proofs true
#print tensorAddAlteration
...
-- syntax (name := solveUsing) term : tactic

-- @[tactic solveUsing]
-- unsafe def evalSolveUsing : Tactic := fun stx => do
--   match stx with
--   | `(tactic| solveUsing $f) => do
--       let fExpr ← Tactic.elabTerm f none
--       dbg_trace fExpr
--       let fType ← inferType fExpr
--       dbg_trace fType
--       let someType ← CheckTactic.CheckGoalType none
--       dbg_trace someType
--       dbg_trace "qua"
--       let optType ← mkAppM ``Option #[someType]
--       dbg_trace "shen"
--       -- dbg_trace s!"{optType}"
--       dbg_trace "le"
--       unless (← isDefEq fType (← mkArrow (mkConst ``Unit) optType)) do
--         throwError "Expected function of type Unit -> Option G"
--       dbg_trace "al"
--       let x ← evalExpr (Unit → Option Expr) (← mkArrow (mkConst ``Unit) optType) fExpr
--       dbg_trace "hello"
--       match x () with
--       | .none => logInfo "solveUsing: function returned none, no progress made."
--       | .some x => do
--         let optVal ← whnf x
...
--         -- assignMVar goal expr
--         replaceMainGoal []
--   | _ => throwUnsupportedSyntax

-- example : Nat := by
--   solveUsing ((λ u : Unit => some 3) : Unit -> Option Nat)
