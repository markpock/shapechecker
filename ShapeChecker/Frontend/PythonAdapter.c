...
lean_object* lean_traverse_function(PyObject* pyFunction) {
    ...
    PyObject* input = PyDict_GetItemString(annotations, "input");

    // Create Lean function object
    lean_object* result = lean_alloc_ctor(0, 4, 32);
    ...
    lean_object* lst = lean_box(0); // nil
    for (int i = 0; i < PyList_Size(input); i++) {
        PyObject* tuple = PyList_GetItem(input, i);
        PyObject* name = PyTuple_GetItem(tuple, 0);
        PyObject* annot = PyTuple_GetItem(tuple, 1);
        lean_object* nameS = lean_mk_string(PyUnicode_AsUTF8(name));
        lean_object* annotS = lean_mk_string(PyUnicode_AsUTF8(annot));
        lean_object* tupleL = lean_alloc_ctor(0, 2, 2 * 8);
        lean_ctor_set(tupleL, 0, nameS);
        lean_ctor_set(tupleL, 1, annotS);
        
        lean_object* lstN = lean_alloc_ctor(1, 2, 2 * 8);
        lean_ctor_set(lstN, 0, tupleL);
        lean_ctor_set(lstN, 1, lst);
        lst = lstN;
    }
    ...
    // PyObject* name = PyTuple_GetItem(input_a, 0);
    // PyObject* annot = PyTuple_GetItem(input_a, 1);
    ...
    printf("  Name: %s\n", PyUnicode_AsUTF8(name));
    printf("  Input annotations changed\n");
